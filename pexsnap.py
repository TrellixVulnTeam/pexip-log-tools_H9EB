...
def extract_snap(snapshot_output, snapshot_input): # extract snapshot
    now = datetime.now()
    ...
    try:
        ...
        enc = True
    if enc:
        decrypt(snapshot_input, snapshot_input.replace(".tgz", "-decrypted.tgz")) # decrypt
    ...
    if not snapshot_input.endswith('.tgz'):
        ...
        sys.exit(2)
    with alive_bar(length=20, title='Decompressing snapshot') as bar_progress:
        try:
            with tarfile.open(snapshot_input) as tar:
                ...
                safe_extract(tar, path=snapshot_output)
                bar_progress()
...
def main():
    ...
    try:
        extract_snap(snapshot_output, snapshot_input) # extract snapshot to folder
        ...
        if worker == False:
            with alive_bar(length=20, title='Processing snapshot...') as bar_progress:
                bar_progress.text(' -- Checking for stability issues')
                if dev_files_array:
                ...
                for script, values in external_scripts.items(): # run external scripts
                    if os.path.isfile(values['script']):
                        if script == 'logreader' and args.skip:
                            ...
                            bar_progress()
                            bar_progress.text(' -- Creating ' + script + ' output')
